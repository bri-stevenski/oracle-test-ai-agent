...
test, promote a test, add a framework), see
...
The central execution engine. Walks the three-stage pipeline
(classify → recommend → generate), the registry-driven
framework selection, pluggable LLM providers, and the
generated-test conventions.

**Best for:** Understanding how a natural-language prompt
becomes a test file.
...
`match_by_language`), and how categories form the public
contract between classifier and recommender.
...
- [Agent Skills](../../agents/skills/README.md) — agent-invokable
  workflows in the SKILL.md format
